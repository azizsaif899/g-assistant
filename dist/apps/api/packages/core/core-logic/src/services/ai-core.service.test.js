...
describe('AiCoreService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                {
                    provide: gemini_client_1.GeminiClient,
                    useValue: mockGeminiClient,
                },
            ],
        }).compile();
        service = module.get(ai_core_service_1.AiCoreService);
        geminiClient = module.get(gemini_client_1.GeminiClient);
    });
    describe('processQuery', () => {
        ...
        it('should build context correctly', async () => {
            ...
            expect(geminiClient.query).toHaveBeenCalledWith(expect.stringContaining('السياق: development'), 'development');
            expect(geminiClient.query).toHaveBeenCalledWith(expect.stringContaining('معرف المستخدم: user123'), 'development');
            expect(geminiClient.query).toHaveBeenCalledWith(expect.stringContaining('الأدوات المتاحة: analyzer, formatter'), 'development');
        });
    });
    describe('analyzeCode', () => {
        it('should analyze code successfully', async () => {
            ...
            expect(geminiClient.query).toHaveBeenCalledWith(expect.stringContaining('```javascript'), 'code_analysis');
        });
    });
    describe('generateJSON', () => {
        it('should generate JSON successfully', async () => {
            ...
            expect(geminiClient.query).toHaveBeenCalledWith(expect.stringContaining('بصيغة JSON صحيحة'), 'json_generation');
        });
    ...
    describe('generateSessionId', () => {
        it('should generate unique session IDs', () => {
            const service1 = new ai_core_service_1.AiCoreService(geminiClient);
            const service2 = new ai_core_service_1.AiCoreService(geminiClient);
            const id1 = service1.generateSessionId();
            const id2 = service2.generateSessionId();
            expect(id1).not.toBe(id2);
            expect(id1).toMatch(/^session_\d+_[a-z0-9]+$/);
        });
    });
});
//# sourceMappingURL=ai-core.service.test.js.map