...
class TestReporter {
    generateConsoleReport(results) {
        const report = [
            ...
            '',
            'Test Suites:',
            ...results.suites.map(suite => `  ${suite.name}: ${suite.passedTests}/${suite.totalTests} passed`)
        ];
        return report.join('\n');
    }
    generateHTMLReport(results, coverage) {
        return `
<!DOCTYPE html>
<html>
<head>
    ...
    generateJUnitXML(results) {
        const xml = [
            ...
            ...results.suites.map(suite => [
                ...
                ...suite.tests.map(test => test.status === 'passed'
