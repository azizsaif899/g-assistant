...
class TestReporter {
    ...
    async generateReport(testResults, coverage) {
        const report = {
    ...
    generateSuiteReports(testResults) {
    ...
    generatePerformanceReport(testResults) {
        const durations = testResults.map((t) => t.duration || 0);
    ...
    calculateTotalDuration(testResults) {
        return testResults.reduce((sum, test) => sum + (test.duration || 0), 0);
    }
    async outputReports(report) {
...
### Slowest Tests
${report.performance.slowestTests.map((test, index) => `${index + 1}. ${test.name}: ${test.duration}ms`).join('\n')}

${report.summary.failed > 0 ? `
## ❌ Failed Tests

${report.suites.flatMap(suite => suite.tests.filter(test => test.status === 'failed')
            .map(test => `- **${suite.name}** > ${test.name}${test.error ? `\n  \`${test.error.message}\`` : ''}`)).join('\n')}
` : ''}
`;
    }
}
