...
const AnalyticsDashboard = () => {
    ...
    return (<div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <KPICard title="المستخدمون النشطون اليوم" value={kpis.userMetrics.dailyActiveUsers.toLocaleString()} trend={{ direction: 'up', percentage: 12.5 }} icon="👥" color="blue"/>
        <KPICard title="الإيرادات الشهرية" value={`$${kpis.businessMetrics.monthlyRecurringRevenue.toLocaleString()}`} trend={{ direction: 'up', percentage: 8.3 }} icon="💰" color="green"/>
        <KPICard title="معدل التحويل" value={`${kpis.businessMetrics.conversionRate.toFixed(1)}%`} trend={{ direction: 'down', percentage: 2.1 }} icon="📈" color="purple"/>
        <KPICard title="وقت الاستجابة" value={`${kpis.technicalMetrics.averageResponseTime}ms`} trend={{ direction: 'up', percentage: 5.2 }} icon="⚡" color="orange"/>
      </div>
...
const KPICard = ({ title, value, trend, icon, color }) => {
    ...
    return (<div className={`rounded-lg border p-6 ${colorClasses[color]}`}>
      ...
      <div className="mt-4 flex items-center">
        <span className={`text-sm ${trend.direction === 'up' ? 'text-green-600' :
...
const ChartCard = ({ title, children }) => (<div className="bg-white rounded-lg shadow p-6">
...
const InsightCard = ({ insight }) => {
    ...
    return (<div className="border rounded-lg p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          {insight.recommendations && insight.recommendations.length > 0 && (<div className="mt-2">
              <p className="text-sm font-medium text-gray-700 mb-1">التوصيات:</p>
              <ul className="text-sm text-gray-600 space-y-1">
                {insight.recommendations.map((rec, index) => (<li key={index} className="flex items-start">
