...
exports.InteractiveKanban = void 0;
...
const InteractiveKanban = () => {
    ...
    const handleDrop = async (targetStageId, deal) => {
        if (deal.stage === targetStageId)
            return;
        ...
        try {
            await event_bus_1.eventBus.publish({
                type: event_bus_1.EventTypes.LEAD_STAGE_CHANGED,
                source: 'kanban-board',
                data: {
                    dealId: deal.id,
    ...
    // عرض اقتراح ذكي
    const showSmartSuggestion = (deal, newStage) => {
        let suggestion = '';
        switch (newStage) {
            case 'qualified':
    ...
    return (<div className="flex space-x-6 rtl:space-x-reverse overflow-x-auto pb-4">
      {stages.map(stage => (<div key={stage.id} className={`min-w-80 ${stage.color} rounded-lg p-4`} onDragOver={(e) => e.preventDefault()} onDrop={(e) => {
                e.preventDefault();
                if (draggedDeal) {
          ...
          {/* قائمة الصفقات */}
          <framer_motion_1.Reorder.Group axis="y" values={stage.deals} onReorder={(newOrder) => {
                setStages(prev => prev.map(s => s.id === stage.id ? { ...s, deals: newOrder } : s));
            }} className="space-y-3">
            {stage.deals.map(deal => (<framer_motion_1.Reorder.Item key={deal.id} value={deal} dragListener={false}>
                <framer_motion_1.motion.div className="bg-white rounded-lg p-4 shadow-sm border cursor-move hover:shadow-md transition-shadow" draggable onDragStart={() => handleDragStart(deal)} onDragEnd={handleDragEnd} whileHover={{ scale: 1.02 }} whileDrag={{ scale: 1.05, rotate: 2 }}>
                  {/* مؤشر الحرارة */}
          ...
          {/* منطقة الإفلات */}
          {draggedDeal && draggedDeal.stage !== stage.id && (<framer_motion_1.motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 60 }} className="border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center mt-3">
              <span className="text-gray-500 text-sm">إفلات هنا</span>
            </framer_motion_1.motion.div>)}
        </div>))}
    </div>);
};
exports.InteractiveKanban = InteractiveKanban;
//# sourceMappingURL=InteractiveKanban.js.map