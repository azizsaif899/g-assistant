...
function useLivePulse() {
    ...
    const { subscribe, isConnected } = (0, useWebSocket_1.useWebSocket)();
    ...
    (0, react_1.useEffect)(() => {
        if (!isConnected)
            return;
        const unsubscribe = subscribe('pulse.update', (data) => {
    ...
    const getHealthStatus = () => {
        const avgHealth = (metrics.system.health +
            metrics.team.performance +
            (100 - metrics.customers.churn * 10) +
            (metrics.deals.conversion * 2)) / 4;
        if (avgHealth >= 90)
            return 'excellent';
        if (avgHealth >= 75)
            return 'good';
    ...
    const getActiveAlertsCount = () => {
        let alerts = 0;
        if (metrics.system.health < 80)
            alerts++;
        if (metrics.system.errors > 3)
            alerts++;
        if (metrics.customers.churn > 4)
            alerts++;
        if (metrics.deals.conversion < 20)
            alerts++;
        if (metrics.team.performance < 70)
            alerts++;
        return alerts;
    };
    return {
...
//# sourceMappingURL=useLivePulse.js.map