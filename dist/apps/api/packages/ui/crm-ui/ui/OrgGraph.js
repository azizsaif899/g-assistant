...
exports.OrgGraph = void 0;
const tslib_1 = require("tslib");
const react_1 = tslib_1.__importStar(require("react"));
const OrgGraph = ({ onNodeClick, onPathFind }) => {
    ...
    const [pathfinderMode, setPathfinderMode] = (0, react_1.useState)(false);
    ...
    return (<div className="org-graph" style={{
            background: 'white',
            borderRadius: '12px',
            border: '1px solid #e0e0e0',
            overflow: 'hidden'
      ...
      <div style={{
        ...
        <svg style={{
          ...
          {edges.map((edge) => {
            ...
            return (<g key={edge.id}>
                <line x1={sourceNode.position.x + 40} y1={sourceNode.position.y + 40} x2={targetNode.position.x + 40} y2={targetNode.position.y + 40} stroke={getRelationshipColor(edge.data.strength)} strokeWidth={Math.max(1, edge.data.strength / 3)} opacity={0.7}/>
                <text x={(sourceNode.position.x + targetNode.position.x) / 2 + 40} y={(sourceNode.position.y + targetNode.position.y) / 2 + 35} fontSize="10" fill="#666" textAnchor="middle">
        ...
        {/* العقد */}
        {nodes.map((node) => (<div key={node.id} onClick={() => handleNodeClick(node)} style={{
                position: 'absolute',
...
exports.OrgGraph = OrgGraph;
//# sourceMappingURL=OrgGraph.js.map