...
exports.PersonalityInsights = PersonalityInsights;
const framer_motion_1 = require("framer-motion");
const lucide_react_1 = require("lucide-react");
function PersonalityInsights({ personality, recommendations }) {
    if (!personality) {
    ...
    const getPersonalityLabel = (type) => {
        switch (type) {
    ...
    return (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-lg shadow-sm border p-6">
      <div className="flex items-center space-x-3 mb-4">
        <lucide_react_1.Brain className="w-5 h-5 text-purple-600"/>
        <h4 className="font-semibold text-gray-900">تحليل الشخصية</h4>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {personality.traits.map((trait, index) => (<span key={index} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-sm">
                {trait}
              </span>))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center space-x-2">
            <lucide_react_1.MessageSquare className="w-4 h-4 text-blue-500"/>
        ...
        {recommendations && recommendations.length > 0 && (<div className="mt-4 p-4 bg-purple-50 rounded-lg border-r-4 border-purple-400">
            <h5 className="font-medium text-purple-900 mb-2">💡 توصيات التواصل</h5>
            <ul className="space-y-1">
              {recommendations.map((rec, index) => (<li key={index} className="text-sm text-purple-800 leading-relaxed">
                  • {rec}
                </li>))}
            </ul>
          </div>)}
...
//# sourceMappingURL=PersonalityInsights.js.map