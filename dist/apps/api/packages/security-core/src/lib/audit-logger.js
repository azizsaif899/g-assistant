...
class AuditLogger {
    ...
    checkMultipleFailedLogins(event) {
        const timeWindow = 15 * 60 * 1000; // 15 minutes
        const threshold = 5;
        const now = new Date();
        const recentFailedLogins = this.events.filter(e => e.action === 'login' &&
            e.outcome === 'failure' &&
    ...
    checkUnusualAccessPattern(event) {
        if (!event.userId)
            return;
    ...
    createSuspiciousActivity(activity) {
        // Check if similar activity already exists
        const existing = this.suspiciousActivities.find(sa => sa.userId === activity.userId &&
    ...
    getSuspiciousActivities(status) {
    ...
    updateSuspiciousActivityStatus(id, status) {
    ...
    generateAuditReport(period = 'daily') {
        const now = new Date();
        let since;
        switch (period) {
            case 'daily':
        ...
        const suspiciousActivitiesCount = this.suspiciousActivities.filter(sa => sa.timestamp >= since).length;
        return {
            period,
