...
class InsightGenerator {
    constructor(dataCollector, metricsProcessor) {
        this.dataCollector = dataCollector;
        this.metricsProcessor = metricsProcessor;
        this.insights = new rxjs_1.BehaviorSubject([]);
        this.config = {
    ...
    // رؤى نمو المستخدمين
    async generateUserGrowthInsights(events, kpis) {
        const insights = [];
        // تحليل معدل النمو
    ...
    // رؤى الإيرادات
    async generateRevenueInsights(events, kpis) {
        const insights = [];
    ...
    // رؤى الأداء التقني
    async generatePerformanceInsights(events, kpis) {
        const insights = [];
        // تحليل وقت الاستجابة
    ...
    // رؤى الذكاء الاصطناعي
    async generateAIInsights(events, kpis) {
        const insights = [];
    ...
    // كشف الشذوذ
    async detectAnomalies(events, kpis) {
        const insights = [];
    ...
    // التنبؤات
    async generatePredictiveInsights(events, kpis) {
        const insights = [];
    ...
    getInsightsByType(type) {
        return this.insights.pipe((0, operators_1.map)(insights => insights.filter(insight => insight.type === type)));
    }
    // الحصول على الرؤى القابلة للتنفيذ
    getActionableInsights() {
        return this.insights.pipe((0, operators_1.map)(insights => insights.filter(insight => insight.actionable && insight.confidence > 0.7)));
    }
    ...
    calculateMRRGrowth(currentMRR) {
        // منطق حساب نمو MRR
        return Math.random() * 0.1; // مؤقت للاختبار
    }
    // كشف شذوذ حركة المرور
    detectTrafficAnomaly(events) {
        // منطق كشف الشذوذ
    ...
    // التنبؤ بنمو المستخدمين
    async predictUserGrowth(events, kpis) {
        // منطق التنبؤ بالنمو
    ...
    // التنبؤ بالإيرادات
    async predictRevenue(events, kpis) {
