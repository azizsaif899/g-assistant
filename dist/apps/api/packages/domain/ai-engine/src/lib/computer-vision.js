...
class ComputerVision {
    ...
    async detectObjects(imageData, threshold = 0.5) {
        const analysis = await this.analyzeImage(imageData);
    ...
    async generateImageDescription(imageData) {
        ...
        return `This image shows ${sceneDescription}. The main objects visible are: ${objects}. ${analysis.faces.length > 0 ? `There are ${analysis.faces.length} face(s) detected.` : ''}`;
    }
